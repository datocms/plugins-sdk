...
- 8c7a2cd: Move to `datocms-structured-text-utils` 6.x, so you don't end up with two copies of it
  
  If your plugin already depends on `datocms-structured-text-utils` 6.x, you have
  been getting a second copy of it — a 5.x one — tucked under datocms-plugin-sdk,
  because we asked for `^5`. Two copies mean a bigger bundle and two versions of
  types that should be one. This release asks for `^6` instead, and the duplicate
  goes away.
